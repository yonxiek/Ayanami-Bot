...
class WebhookCommands(commands.Cog):
    ...
    webhook = app_commands.Group(name="webhook", description="Управление вебхуками")

    @webhook.command(name="create", description="Создать вебхук и сразу открыть конструктор")
    @app_commands.describe(channel="Канал", name="Имя вебхука")
    @app_commands.default_permissions(manage_webhooks=True)
    async def webhook_create(self, interaction: discord.Interaction, channel: discord.TextChannel, name: str = "Ayanami Webhook"):
        await interaction.response.defer(ephemeral=True)
        try:
            webhook = await channel.create_webhook(name=name, reason=f"Создан {interaction.user}")
        except discord.Forbidden:
    ...
    @webhook.command(name="build", description="Открыть конструктор для существующего вебхука")
    @app_commands.describe(webhook_url="URL вебхука")
    @app_commands.default_permissions(manage_webhooks=True)
    async def webhook_build(self, interaction: discord.Interaction, webhook_url: str):
        view = WebhookBuilderView(webhook_url)
        embed = view._make_embed()
        await interaction.response.send_message(embed=embed, view=view, ephemeral=True)

    @webhook.command(name="send", description="Быстро отправить сообщение через вебхук")
    @app_commands.describe(webhook_url="URL вебхука", message="Текст сообщения", embed_title="Заголовок embed", embed_description="Описание embed", embed_color="Цвет hex (#ff0000)")
    @app_commands.default_permissions(manage_webhooks=True)
        ...
        if embed_title or embed_description:
            ...
            if embed_color:
                try:
                    ...
                    pass
            embed = discord.Embed(title=embed_title or "", description=embed_description or "", color=color)
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(webhook_url, json=data) as resp:
                if resp.status in (200, 204):
                    ...
                    text = await resp.text()
                    await interaction.followup.send(f"❌ Ошибка {resp.status}: `{text[:300]}`", ephemeral=True)

    @webhook.command(name="template", description="Получить шаблон JSON")
    @app_commands.describe(template_type="Тип шаблона")
    @app_commands.choices(template_type=[
        app_commands.Choice(name="Embed (полный)", value="embed_full"),
        app_commands.Choice(name="Embed (минимальный)", value="embed_simple"),
        app_commands.Choice(name="Кнопки (link)", value="buttons_link"),
        app_commands.Choice(name="Полный payload", value="full_payload"),
    ])
    async def webhook_template(self, interaction: discord.Interaction, template_type: app_commands.Choice[str]):
        import json as _json
        templates = {
        ...
        raw = _json.dumps(templates.get(template_type.value, {}), ensure_ascii=False, indent=2)
        await interaction.response.send_message(f"📋 **Шаблон: {template_type.name}**\n```json\n{raw}\n```", ephemeral=True)

    @webhook.command(name="list", description="Показать вебхуки канала")
    ...
    @webhook.command(name="delete", description="Удалить вебхук")
    @app_commands.describe(webhook_url="URL вебхука")
    @app_commands.default_permissions(manage_webhooks=True)
    async def webhook_delete(self, interaction: discord.Interaction, webhook_url: str):
        await interaction.response.defer(ephemeral=True)
        import aiohttp
        async with aiohttp.ClientSession() as session:
